 ...
 * `~/.grok/sessions/**\/updates.jsonl`, `~/.wolf/agent/sessions/**\/*.jsonl`)
 ...
 *
 * Each bump since v4 only adds a provider to {@link UsageProviderKind}, so
 * older buckets remain valid and mixed-version environments keep those totals
 * instead of treating every older server as stale.
 */
export const USAGE_MERGE_COMPATIBLE_SINCE = 4 as const;

export const UsageProviderKind = Schema.Literals(["claude", "codex", "grok", "wolf"]);
