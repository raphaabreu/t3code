...
const AUTOMATIC_CREDENTIAL_SIGNAL_PAIRING_MS = 2 * 60 * 1_000;
const AUTOMATIC_FAILOVER_MESSAGE_PREFIX = "[Automatic credential failover]";

type CredentialFailureSignal = {
  readonly reason: "usage-limit" | "access-denied";
  readonly instanceId: ProviderInstanceId;
  readonly retryAt?: number | undefined;
};

type RememberedCredentialFailureSignal = CredentialFailureSignal & {
...
function isPausedClaudeUsageLimit(event: ProviderRuntimeEvent): boolean {
  if (
    ...
    return false;
  const detail = asUnknownRecord(event.payload.detail);
  return (
    ...
    isExplicitUsageLimitMessage(event.payload.message)
  );
}
...
export function credentialFailureSignalFromRuntimeEvent(
  event: ProviderRuntimeEvent,
): CredentialFailureSignal | undefined {
  const instanceId = event.providerInstanceId;
  if (!instanceId) return undefined;
  if (event.type === "runtime.warning" && isPausedClaudeUsageLimit(event)) {
    return { instanceId, reason: "usage-limit", retryAt: findRetryAt(event.payload.detail) };
  }
  const message =
  ...
  // Claude emits this account-level denial as an API error, including in a
  // success-shaped SDK result. Generic 403/tool permission errors are unrelated.
  if (
    event.provider === "claudeAgent" &&
    /\byour organization has disabled claude subscription access for claude code\b/i.test(message)
  ) {
    return { instanceId, reason: "access-denied" };
  }
  if (isExplicitUsageLimitMessage(message)) {
    const retryAt = findRetryAt(
      event.type === "runtime.error"
    ...
    return { instanceId, reason: "usage-limit", ...(retryAt !== undefined ? { retryAt } : {}) };
  }
  return undefined;
}
...
const make = Effect.gen(function* () {
  ...
  const credentialFailureSignalsByThread = new Map<ThreadId, RememberedCredentialFailureSignal>();
  ...
  const maybeHandleAutomaticCredentialFailover = Effect.fn(
    ...
    const directSignal = credentialFailureSignalFromRuntimeEvent(event);
    if (directSignal) {
      const now = yield* Clock.currentTimeMillis;
      const previousSignal = credentialFailureSignalsByThread.get(event.threadId);
      ...
      credentialFailureSignalsByThread.set(event.threadId, {
        ...directSignal,
        retryAt,
        observedAt: now,
    ...
    const isPausedLimit = isPausedClaudeUsageLimit(event);
    ...
    const now = yield* Clock.currentTimeMillis;
    const rememberedCandidate = credentialFailureSignalsByThread.get(event.threadId);
    const rememberedSignal =
      ...
      (rememberedCandidate.turnId === event.turnId ||
        ((rememberedCandidate.turnId === undefined || event.turnId === undefined) &&
          now - rememberedCandidate.observedAt <= AUTOMATIC_CREDENTIAL_SIGNAL_PAIRING_MS))
        ? rememberedCandidate
        : undefined;
    if (rememberedCandidate && !rememberedSignal) {
      credentialFailureSignalsByThread.delete(event.threadId);
    }
    ...
    if (isPausedLimit && thread.session?.activeTurnId !== event.turnId) return;
    ...
    if (isPausedLimit) {
      // Claude parks inside the SDK without a terminal event. Stop that writer
      // and finalize the interrupted turn before requesting a replacement.
      yield* providerService.stopSession({ threadId: thread.id });
      yield* processRuntimeEvent({
        ...event,
        eventId: EventId.make(`${event.eventId}:auto-limit-stopped`),
        type: "turn.completed",
    ...
    const continuationMessage = [
      ...
      signal.reason === "access-denied"
        ? "The previous credential does not have subscription access to Claude Code."
        : "The previous credential reached its usage limit.",
      "Continue the interrupted request from the last completed step without repeating completed work.",
